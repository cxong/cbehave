...
FEATURE(mock_macros, "CBehave mock macros test")
    SCENARIO("Invoke bar_invoke_foo_once, within which foo would be invoked only once")
        ...
        THEN("Bar_invoke_foo_once should return 7")
            SHOULD_INT_EQUAL(ret, 7);
        AND("its outparameter should be 5!")
        ...
        THEN("Bar_invoke_foo_once should return 13")
            SHOULD_INT_EQUAL(ret, 13);
        AND("its outparameter should be 17!")
    ...
    SCENARIO("Invoke bar_invoke_foo_multi_times")
        ...
        THEN("Bar_invoke_foo_multi_times should return 39")
            SHOULD_INT_EQUAL(ret, 39);
        AND("its outparameter should be 51!")
